...
function mapRecord(page) {
  ...
  const time = plainText(prop(p, ['Tijd', 'Time', 'Tijdstip']));
  ...
  return { slug, ensemble, title, date: dateValue, time, startTime, endTime, venue, city, description, ticketUrl, image };
}
...
export function getArchivedConcerts(concerts, today = todayInNetherlands()) {
  return concerts
    ...
    .sort((a, b) => `${b.date || ''}${b.time || b.startTime || ''}`.localeCompare(`${a.date || ''}${a.time || a.startTime || ''}`));
}
